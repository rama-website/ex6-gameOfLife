...
function createCell(x, y) {
  ...
  return {
    ...
    y,
    alive,
    lifeTime: alive ? 1 : 0, // Initialize lifeTime based on cell's initial state
  };
}
...
function createGame(context, numRows, numColumns) {
  ...
  function drawCell(cell) {
    // Determine opacity based on lifeTime
    let opacity;
    ...
    // Draw cell background with adjusted opacity
  ...
  function updateGrid() {
    ...
    forEachCell((cell) => {
      ...
      // Update lifeTime when transitioning between states
      if (cell.alive !== cell.nextAlive) {
        cell.lifeTime = cell.nextAlive ? 1 : 0;
      }
    });
